...
// Global API server endpoint - pointing to live dev environment
const String apiBaseUrl = 'https://ais-dev-v7qaidrhbnnq6zxcjftgug-525007708281.asia-southeast1.run.app';
...
const Map<String, Map<String, String>> translations = {
  'en': {
    ...
    'grievanceBacklog': 'Grievance Backlog',
  ...
  'hi': {
    ...
    'grievanceBacklog': 'लंबित शिकायतें',
    'projectPlanner': 'परियोजना योजनाकार',
    'resolve': 'सुलझाएं',
    'resolved': 'सुलझ गया',
    'filterStatus': 'स्थिति बदलें',
    'filterCategory': 'श्रेणी बदलें',
    'activeHotspots': 'सक्रिय हॉटस्पॉट',
    'totalReports': 'कुल मामले',
  },
  'hinglish': {
  ...
  Future<void> _submitGrievance() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('$apiBaseUrl/api/grievance'), // Pointing to live MP server
  ...
  Future<void> _syncOfflineQueue() async {
    ...
    for (var item in queue) {
      try {
        ...
        final response = await http.post(
          Uri.parse('$apiBaseUrl/api/grievance'),
          headers: {'Content-Type': 'application/json'},
...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  int _selectedTab = 0; // 0 = Backlog, 1 = Planner
  List<dynamic> _grievances = [];
  bool _isLoadingGrievances = false;
  ...
  Future<void> _fetchGrievances() async {
    setState(() {
      _isLoadingGrievances = true;
      _grievanceError = '';
    });
    try {
      final response = await http.get(Uri.parse('$apiBaseUrl/api/grievances')).timeout(const Duration(seconds: 12));
      if (response.statusCode == 200) {
        final Map<String, dynamic> data = jsonDecode(response.body);
        setState(() {
          _grievances = data['grievances'] ?? [];
      ...
      setState(() {
        _grievanceError = 'Failed to load live grievances from the Member of Parliament server. Check connection.';
        _isLoadingGrievances = false;
      });
    }
  }

  Future<void> _resolveGrievance(String id) async {
    setState(() {
      _isLoadingGrievances = true;
    });
    try {
      final response = await http.post(
        Uri.parse('$apiBaseUrl/api/grievance/resolve'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'id': id,
          'adminLang': widget.currentLanguage == 'hi' ? 'hi' : 'en',
        }),
      ).timeout(const Duration(seconds: 10));

      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Grievance marked as resolved & citizen notified!'), backgroundColor: Colors.green),
        );
        _fetchGrievances();
      } else {
        throw Exception("Failed to resolve on server");
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  IconButton(
                    icon: const Icon(Icons.refresh_rounded, size: 20, color: Color(0xFF4F46E5)),
                    ...
                    Expanded(
                      child: GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Text(
                            widget.t('grievanceBacklog').toUpperCase(),
                            style: TextStyle(
                              fontSize: 10,
                              fontWeight: FontWeight.bold,
                              color: _selectedTab == 0 ? const Color(0xFF0F172A) : Colors.slate.shade500,
                            ),
                          ),
                        ),
                      ),
                    ...
                    Expanded(
                      child: GestureDetector(
                        ...
                        child: Container(
                          ...
                          alignment: Alignment.center,
                          child: Text(
                            widget.t('projectPlanner').toUpperCase(),
                            style: TextStyle(
                              fontSize: 10,
                              fontWeight: FontWeight.bold,
                              color: _selectedTab == 1 ? const Color(0xFF0F172A) : Colors.slate.shade500,
                            ),
                          ),
                        ),
                      ),
        ...
        // Main body content switching based on Tab
        Expanded(
          child: _selectedTab == 0 ? _buildGrievanceBacklogTab() : _buildProjectPlannerTab(),
        ),
      ],
    );
  }

  Widget _buildGrievanceBacklogTab() {
    if (_isLoadingGrievances && _grievances.isEmpty) {
      return const Center(
        child: CircularProgressIndicator(color: Color(0xFF4F46E5)),
      );
    }

    if (_grievanceError.isNotEmpty && _grievances.isEmpty) {
      return Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Icon(Icons.cloud_off_rounded, size: 48, color: Colors.slate),
              const SizedBox(height: 12),
              Text(
                _grievanceError,
                textAlign: TextAlign.center,
    ...
    final filteredGrievances = _grievances.where((g) {
    ...
    // Hotspot computation
    final hotspotCount = _grievances.where((g) {
      final count = g['trafficCount'] ?? 1;
      return count > 1;
    }).length;
        ...
        children: [
            ...
            children: [
              Expanded(
                child: _buildStatCard(
                  widget.t('totalReports').toUpperCase(),
                  '${_grievances.length}',
              ...
              Expanded(
                child: _buildStatCard(
                  widget.t('activeHotspots').toUpperCase(),
                  '$hotspotCount',
          ...
          if (filteredGrievances.isEmpty)
            Container(
              ...
              child: Column(
                children: [
                  Icon(Icons.assignment_turned_in_rounded, size: 36, color: Colors.slate.shade300),
                  const SizedBox(height: 10),
                  Text(
                    'No $_statusFilter grievances found.',
                    style: TextStyle(fontSize: 12, color: Colors.slate.shade500, fontWeight: FontWeight.bold),
                  ),
                ],
              ),
            )
            ...
            ListView.builder(
              ...
              itemBuilder: (context, idx) {
                final item = filteredGrievances[idx];
  ...
  Widget _buildStatCard(String title, String value, Color color, IconData icon) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.slate.shade50, blurRadius: 2, offset: const Offset(0, 1))],
      ),
      child: Row(
        children: [
          Container(
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(title, style: TextStyle(fontSize: 8, fontWeight: FontWeight.bold, color: Colors.slate.shade500, letterSpacing: 0.5)),
                const SizedBox(height: 2),
  ...
  Widget _buildGrievanceCard(dynamic item) {
    final id = item['id'] ?? '';
    final name = item['name'] ?? 'Citizen';
    final contact = item['contact'] ?? 'N/A';
    final department = item['department'] ?? 'Other';
    final location = item['cleanLocation'] ?? 'Delhi NCR';
    final desc = item['description'] ?? 'No description';
    final status = item['status'] ?? 'Open';
    final trafficCount = item['trafficCount'] ?? 1;
    ...
    if (createdAt.isNotEmpty) {
      try {
        ...
        displayDate = DateFormat('dd MMM yyyy, hh:mm a').format(parsed);
      } catch (_) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(12),
        side: BorderSide(color: Colors.slate.shade150),
      ),
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                  decoration: BoxDecoration(
                    color: isResolved ? Colors.emerald.shade50 : Colors.amber.shade50,
                    borderRadius: BorderRadius.circular(6),
                  ),
                  child: Text(
                    isResolved ? widget.t('resolved').toUpperCase() : 'OPEN',
                    style: TextStyle(
                      fontSize: 8,
                      fontWeight: FontWeight.bold,
                      color: isResolved ? Colors.emerald.shade700 : Colors.amber.shade700,
                    ),
                  ),
                ),
              ],
            ...
            // Metadata Lines (Landmark, Date, Reporter)
            Row(
              children: [
                const Icon(Icons.location_on_rounded, size: 11, color: Colors.slate),
                const SizedBox(width: 4),
                Expanded(
                  child: Text(
                    location,
                    ...
                    style: const TextStyle(fontSize: 10, color: Colors.slate, fontWeight: FontWeight.w500),
                  ),
                ),
              ],
            ),
            ...
            Row(
              children: [
                const Icon(Icons.person_rounded, size: 11, color: Colors.slate),
                const SizedBox(width: 4),
                Expanded(
                  child: Text(
                    '$name ($contact)',
            ...
            if (displayDate.isNotEmpty) ...[
              ...
              Row(
                children: [
                  const Icon(Icons.access_time_filled_rounded, size: 11, color: Colors.slate),
                  const SizedBox(width: 4),
                  Text(
                    displayDate,
            ...
            // Actions or Duplicate Badge Footer
            Row(
              mainAxisAlignment: MainAxisAlignment.between,
              children: [
                // Duplicate Traffic Indicator
                Row(
                  children: [
                    Icon(
                      trafficCount > 1 ? Icons.local_fire_department_rounded : Icons.person_add_alt_1_rounded,
                      size: 13,
                      color: trafficCount > 1 ? Colors.redAccent : Colors.indigo,
                    ),
                    const SizedBox(width: 4),
                    Text(
                      '$trafficCount ${trafficCount > 1 ? "Reporter Hotspot!" : "Reporter"}',
                      style: TextStyle(
                        fontSize: 9,
                        fontWeight: FontWeight.bold,
                        color: trafficCount > 1 ? Colors.redAccent.shade700 : Colors.slate.shade600,
                      ),
                    ),
                  ],
                ),
                ...
                if (!isResolved)
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: () => _resolveGrievance(id),
                      icon: const Icon(Icons.check_circle_outline_rounded, size: 11),
                      label: Text(
                        widget.t('resolve').toUpperCase(),
                  ...
                  const Row(
                    children: [
                      Icon(Icons.cloud_done_rounded, size: 12, color: Colors.emerald),
                      SizedBox(width: 4),
                      Text(
                        'NOTIFIED VIA SMS',
  ...
  Widget _buildProjectPlannerTab() {
    return SingleChildScrollView(
      physics: const BouncingScrollPhysics(),
      padding: const EdgeInsets.all(16.0),
      child: Column(
